...
public class SampleProducer extends Thread {
	...
	public SampleProducer(String topic, Boolean isAsync, String payLoad) {
	...
	public void run() {
        int messageNo = 1;
		while (true) {
			String msgStr = "Message_" + messageNo;
			long startTime = System.currentTimeMillis();
			if (isAsync) { // Send asynchronously
				producer.send(new ProducerRecord(topic, messageNo, payLoad),
						new DemoCallBack(startTime, messageNo, payLoad));
			} else { // Send synchronously
				try {
					producer.send(new ProducerRecord(topic, messageNo, payLoad)).get();

					System.out.println("Sent message: (" + messageNo + ", " + payLoad + ")");
    ...
    public void sendKafkaMessage(String payload,
             String topic)
    {
        System.out.println("info Sending Kafka message: " + payload);
